...
describe UnicodeCaseConverter do
  ...
  describe '#initialize' do
    it 'raises an error if the text argument is nil' do
      -> { expect(UnicodeCaseConverter::upcase(nil)).to raise_error }
    end

    it 'raises an error if the text is not a String' do
      -> { expect(UnicodeCaseConverter::downcase(5)).to raise_error }
    end
  end

  it "upcases a string (example)" do
    expect(UnicodeCaseConverter::upcase("αβγδεζηθικλμνξοπρστυφχψωάέήίόύώϊϋ")).to eq("ΑΒΓΔΕΖΗΘΙΚΛΜΝΞΟΠΡΣΤΥΦΧΨΩΆΈΉΊΌΎΏΪΫ")
  end

  it "downcases a string (example)" do
    expect(UnicodeCaseConverter::downcase("ΑΒΓΔΕΖΗΘΙΚΛΜΝΞΟΠΡΣΤΥΦΧΨΩΆΈΉΊΌΎΏΪΫ")).to eq("αβγδεζηθικλμνξοπρστυφχψωάέήίόύώϊϋ")
  end

  it "capitalizes a string (example)" do
    text = "ΑΒΓΔΕΖΗΘΙΚΛΜΝΞΟΠΡΣΤΥΦΧΨΩΆΈΉΊΌΎΏΪΫ".freeze
    unicode_result = Unicode.capitalize(text)
    expect(UnicodeCaseConverter::capitalize(text)).to eq(unicode_result)
  end
    ...
    [*'0'..'9', *'A'..'F'].sample(16).sort.each do |second_digit|
      [*'0'..'9', *'A'..'F'].sample(16).sort.each do |third_digit|
        [*'0'..'9', *'A'..'F'].sample(16).sort.each do |forth_digit|
          ...
          it "upcases a string: #{text}" do
            unicode_result = Unicode.upcase(text.dup)
            ucc = UnicodeCaseConverter::upcase(text.dup)
            expect(ucc).to eq(unicode_result)
          end
        end
      end
    ...
    [*'0'..'9', *'A'..'F'].sample(16).sort.each do |second_digit|
      [*'0'..'9', *'A'..'F'].sample(16).sort.each do |third_digit|
        [*'0'..'9', *'A'..'F'].sample(16).sort.each do |forth_digit|
          ...
          it "downcases a string: #{text}" do
            unicode_result = Unicode.downcase(text)
            ucc = UnicodeCaseConverter::downcase(text)
            expect(ucc).to eq(unicode_result)
          end
        end
      end
  ...
  #         UnicodeCaseConverter::downcase(string * 100)
  #       end
  #       puts StackProf::Report(data).print_text
  #     end
  #   end
  # end
...
#   end
  ...
  it 'is fast' do
    ...
    benchmark do
      5.times do
        UnicodeCaseConverter::downcase(string)
      end
      5.times do
        UnicodeCaseConverter::upcase(string)
      end
      5.times do
        UnicodeCaseConverter::capitalize(string)
      end
    end
  end
